...
class Estimator:
    """ Does performance estimation of an Allo `Schedule` """
    ...
    def __init__(self, s:Schedule, input_dir:str|Path='input'):
        ...
        self.prj_path = 'none'
        self.estm_model = LLM(GFLASH)       # Does performance estimation
        self.critic_model = LLM(GFLASH)     # criticizes `estm_model`
        self.extract_model = LLM(GLITE)     # Extracts numbers from raw answers
        self.estm_model.system_prompt = \
'''
...
'''
        self.critic_model.system_prompt = \
'''
You are a critisizing tool. You attack an analysis. You think like this:

1. Read through the analysis in detail and identify all its weak points.
2. List these weak points and explain why they are weak.
2. Provide constructive suggestions for improvement.

Be objective and unemotional, without words such as 'please', 'apologize', etc.
'''
...
'''
    ...
    R = TypeVar('R')
    def _estm_crit_extract(self, estm_prompt:str, extract_prompt:str,
                           output_convert:Callable[...,R],
                           criticize_pass_count=2,
                           extract_tries=3) -> tuple[R, str]:
        """ Runs `self.estm_model`, then `self.extract_model`. 
                `model.files` are automatically added to the prompts.
        
        :param estm_prompt: overwrites the user prompt for `self.estm_model`.
        :param extract_prompt: overwrites the user prompt for 
                `self.extract_model`.
        :param output_convert: callable that converts the output string.
        :param criticize_pass_count: (optional) how many times `critic_model`
                should be run.
        :param extract_tries: (optional) re-run count for
                `self.extract_model` before failing.

        :return: tuple of 1) the converted output, passed
                through output_convert. None if
                conversion fails; 2) raw output of `self.estm_model`.

        :raise warning: fails to produce answer that is convertible by
                `output_convert`.
        """
        if criticize_pass_count > 5:
            raise ValueError(f'criticize_pass_count too large: max 5, got {criticize_pass_count}')
        if extract_tries > 5:
            raise ValueError(f'extract_tries too large: max 5, got {extract_tries}')
        
        # Estimator-critic loop
        conversation_history = 'Conversation history (oldest first):\n'
        conversation_history += '\n<role>User</role>:\n'
        conversation_history += estm_prompt
        conversation_history += '\n<role>Large language model (estimator)</role>:\n'
        self.estm_model.user_prompt = conversation_history
        estimator_output = self.estm_model.generate()
        conversation_history += estimator_output
        for _ in range(criticize_pass_count):
            conversation_history += '\n<role>User</role>:\n'
            conversation_history += \
'''
criticize this analysis. Attack its weak points. Provide suggestions for improvement. If the analysis is completely fine, reply with this exact string (without quotation marks): 'Fine.'.
'''
            conversation_history += '\n<role>Large language model (criticizer)</role>:\n'
            self.critic_model.user_prompt = conversation_history
            critic_output = self.critic_model.generate()
            if len(critic_output) < 10:
                if critic_output.strip().strip('.').lower() == 'fine':
                    break
            conversation_history += critic_output
            conversation_history += '\n<role>User</role>:\n'
            conversation_history += \
'''
Improve your answer based on the criticism received. Do not be stubborn.
'''
            conversation_history += '\n<role>Large language model (estimator)</role>:\n'
            self.estm_model.user_prompt = conversation_history
            estimator_output = self.estm_model.generate()
        ...
        # Extractor-parser loop
        extract_prompt += \
            f'\n\nHere is the raw text:\n\n{estimator_output}'
        wrong_answers = set()
        for _ in range(extract_tries):
            hint = ''
            if wrong_answers:
            ...
            try:
                return output_convert(extract_output.rstrip().splitlines()[-1]), estimator_output
...
'''
        estm_prompt += str(self.s.module)
        self.estm_model.add_files([self.file_input_dir / 'example.mlir'])
...
1073741824
'''
        return self._estm_crit_extract(estm_prompt, 
                                       extract_prompt,
                                       lambda x: int(x),
                                       criticize_pass_count=0)[0]
...
8594128896
'''
        return self._estm_crit_extract(estm_prompt, extract_prompt,
                                  lambda x: int(x), criticize_pass_count=0)[0]

    def _run_csynth(self):
        """ Generates the Vivado HLS project directory.
        Updates `self.prj_name`. May take ~1-5 minutes.
    ...
    def _get_hls_rpts(self, prj_path:Path|str) -> list[Path]:
    ...
    def _get_clk_period(self, prj_path:Path|str) -> float|None:
    ...
    def _get_cycles_per_kernel(self,
                               attach_paths:list[str|Path]) -> float:
...
'''
        self.estm_model.add_files(attach_paths)
        extract_prompt = \
r'''
...
'''
    ...
    def get_seconds_per_kernel(self, prj_path:Path|str=None) -> float|None:
        """ Gets latency in seconds per kernel. Runs csynth and LLM.

        :param prj_path: (optional) path of the project directory. Tries to
                skip csynth if provided.

        :return: latency in seconds per kernel, or `None` if failed.
        """
        # Run csynth if and only if no csynth dirs exist
        self.prj_path = self.prj_path if prj_path is None else prj_path
        if not Path(self.prj_path).is_dir():
            csynth_exit_state = self._run_csynth()
            if csynth_exit_state != 0:
                logger.error('Csynth failed.')
                return None
        prj_files_paths = self._get_hls_sources(self.prj_path)
        prj_files_paths.extend(self._get_hls_rpts(self.prj_path))
        seconds_per_cycle = self._get_clk_period(self.prj_path)
    ...
    def get_coords(self, prj_path:Path|str=None) -> tuple[float,float]:
        ...
        try:
            ...
            # TODO: Keep source code and report files if
            # we are caching csynth results.
            if (self.prj_path / 'out.prj/solution1/.autopilot').is_dir():
                shutil.rmtree(self.prj_path, ignore_errors=True)
