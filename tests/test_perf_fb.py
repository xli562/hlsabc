...
def test_rmse():
    ...
    np.testing.assert_allclose(got, [0.33489057,0.5], rtol=1e-5)
...
def test_suggestion():
    perf_fb = PerfFB('tests/resource')
    suggestion = perf_fb.suggestion()
    logger.debug(f'\n{suggestion}')
    assert isinstance(suggestion, str), f'Expects type str, got type {type(suggestion)}'
